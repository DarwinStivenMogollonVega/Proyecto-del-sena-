...
<div class="modal fade" id="{{ $modalId }}" tabindex="-1" role="dialog" aria-labelledby="{{ $modalId }}Label" data-bs-backdrop="static" data-bs-keyboard="false">
        ...
        <div class="modal-content bg-white p-4" style="border-radius:1.2rem; box-shadow:0 12px 36px rgba(0,0,0,.12);">
            <form action="{{ $actionResolved }}" method="post" aria-describedby="{{ $modalId }}Label">
                ...
                <div class="modal-header border-0 pb-0">
                    <h4 class="modal-title fw-bold" id="{{ $modalId }}Label" style="color:{{ $isActive ? 'var(--adm-danger)' : '#0f8a3e' }}; font-size:1.4rem;">{{ $titleText }}</h4>
                ...
                <div class="modal-footer justify-content-center border-0 pt-0">
                    <div class="d-flex justify-content-center gap-3 w-100">
                        <button type="button" class="btn btn-secondary px-4 py-2" style="font-size:1rem; border-radius:.6rem; min-width:120px;" data-bs-dismiss="modal">{{ $cancelText }}</button>
                        <button type="submit" class="btn {{ $isActive ? 'btn-danger' : 'btn-success' }} px-4 py-2" style="font-size:1rem; border-radius:.6rem; min-width:120px;">{{ $buttonText }}</button>
                    </div>
                </div>
            </form>
        </div>
