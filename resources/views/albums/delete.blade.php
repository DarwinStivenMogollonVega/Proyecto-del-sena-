@php
    $actionResolved = $action ?? (isset($reg) ? route('albums.destroy', $reg->getKey()) : '#');
    $modalId = $modalId ?? (isset($reg) ? 'modal-eliminar-album-'.$reg->getKey() : 'modal-eliminar-album');
    $titleText = $title ?? (isset($reg) ? "Eliminar: {$reg->nombre}" : 'Confirmar eliminación');
    $messageText = $message ?? (isset($reg) ? "Esta acción es irreversible. Confirma eliminar el álbum: {$reg->nombre}." : 'Esta acción es irreversible. ¿Deseas continuar?');
@endphp
        ...
        <div class="modal-content bg-white p-4" style="border-radius:1.2rem; box-shadow:0 8px 32px rgba(0,0,0,.18);">
            <form action="{{ $actionResolved }}" method="post" aria-describedby="{{ $modalId }}Label">
                ...
                <div class="modal-header border-0 pb-0">
                    <h4 class="modal-title fw-bold" id="{{ $modalId }}Label" style="color:#b91c1c; font-size:1.5rem;">{{ $title ?? 'Eliminar álbum' }}</h4>
                ...
                <div class="modal-body" style="color:#222; font-size:1.02rem;">
                    <div class="mb-3" style="background:#f7fbfd; border:1px solid #e6eef4; border-radius:.8rem; padding:1rem; box-shadow:0 2px 6px rgba(14,30,37,0.03);">
                        <strong>Advertencia:</strong> Esta acción es <u>irreversible</u>. Por favor, confirme que realmente desea eliminar el álbum <strong>{{ $reg->nombre ?? $reg->name }}</strong>.
                    </div>
                    <p class="mb-0 text-center">¿Usted desea eliminar el álbum <strong>{{ $reg->nombre ?? $reg->name }}</strong>?</p>
                </div>
