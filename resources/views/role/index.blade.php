    ...
    <div class="container-fluid">
            ...
            <div class="d-flex gap-2">
                <form action="{{ route('roles.index') }}" method="get" class="d-flex">
                    <div class="input-group">
                        <input name="texto" type="text" class="form-control form-control-sm" value="{{ $texto }}" placeholder="Buscar rol por nombre">
                        <button type="submit" class="btn btn-outline-primary btn-sm"><i class="bi bi-search"></i></button>
                    </div>
                </form>
                @can('rol-create')
                    <a href="{{ route('roles.create') }}" class="btn btn-sm btn-primary">Nuevo</a>
                @endcan
            </div>
        </div>
        ...
        <div class="card mb-4" style="border-radius:1rem">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if($reg->permissions->isNotEmpty())
                                            {!! $reg->permissions->pluck('name')->map(function($name) { return "<span class='badge panel-badge-primary me-1'>".e($name)."</span>"; })->implode(' ') !!}
                                        @else
                                    ...
                                    <td class="actions-col">
                                        <div class="d-flex gap-2 justify-content-end">
                                            @can('rol-edit')
                                            <a href="{{ route('roles.edit', $reg->getKey()) }}" class="btn btn-sm btn-outline-info" title="Editar"><i class="bi bi-pencil"></i></a>
                                            @endcan
                                            @can('rol-delete')
                                            <button class="btn btn-sm btn-outline-danger" data-bs-toggle="modal" data-bs-target="#modal-eliminar-role-{{ $reg->getKey() }}" title="Eliminar"><i class="bi bi-trash"></i></button>
                                            @endcan
                                        </div>
                                    </td>
                                </tr>
            ...
            <div class="card-footer d-flex justify-content-between align-items-center">
                <div>Mostrando {{ $registros->count() }} de {{ $registros->total() }} roles</div>
