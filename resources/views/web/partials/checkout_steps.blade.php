{{-- resources/views/web/partials/checkout_steps.blade.php --}}
<div class="checkout-steps-bg w-100">
    ...
    @foreach ($steps as $i => $step)
        <div class="stepper-item text-center flex-fill">
            <div class="stepper-circle mb-1 {{ isset($currentStep) && $currentStep == $i ? 'active' : (isset($currentStep) && $currentStep > $i ? 'completed' : '') }}">
                <i class="bi {{ $step['icon'] }}"></i>
            </div>
            <div class="stepper-label {{ isset($currentStep) && $currentStep == $i ? 'active' : (isset($currentStep) && $currentStep > $i ? 'completed' : '') }}">
                {{ $step['label'] }}
            </div>
        </div>
        @if ($i < count($steps) - 1)
            <div class="stepper-line flex-grow-1 align-self-center {{ isset($currentStep) && $currentStep > $i ? 'completed' : '' }}"></div>
        @endif
    @endforeach
