<?php
    ...
    $modalId = $modalId ?? (isset($reg) ? 'modal-eliminar-usuario-'.$reg->getKey() : 'modal-eliminar-usuario');
    $titleText = $title ?? (isset($reg) ? "Eliminar: {$reg->name}" : 'Confirmar eliminación');
    $messageText = $message ?? (isset($reg) ? "Esta acción es irreversible. Confirma eliminar el usuario: {$reg->name}." : 'Esta acción es irreversible. ¿Deseas continuar?');
?>
        ...
        <div class="modal-content bg-white p-4" style="border-radius:1.2rem; box-shadow:0 8px 32px rgba(0,0,0,.18);">
            <form action="<?php echo e($actionResolved); ?>" method="POST" aria-describedby="<?php echo e($modalId); ?>Label">
                ...
                <div class="modal-header border-0 pb-0">
                    <h4 class="modal-title fw-bold" id="<?php echo e($modalId); ?>Label" style="color:#b91c1c; font-size:1.5rem;"><?php echo e($title ?? 'Eliminar usuario'); ?></h4>
                ...
                <div class="modal-body" style="color:#222; font-size:1.02rem;">
                    <div class="mb-3" style="background:#f7fbfd; border:1px solid #e6eef4; border-radius:.8rem; padding:1rem; box-shadow:0 2px 6px rgba(14,30,37,0.03);">
                        <strong>Advertencia:</strong> Esta acción es <u>irreversible</u>. Por favor, confirme que realmente desea eliminar el usuario <strong><?php echo e($reg->name ?? $reg->nombre); ?></strong>.
                    </div>
                    <p class="mb-0 text-center">¿Usted desea eliminar el usuario <strong><?php echo e($reg->name ?? $reg->nombre); ?></strong>?</p>
                </div>
...
<?php if (! $__env->hasRenderedOnce('382915c7-fe05-4eb0-9d0a-b300016bf307')): $__env->markAsRenderedOnce('382915c7-fe05-4eb0-9d0a-b300016bf307'); ?>
    ...
    <?php $__env->stopPush(); ?>
<?php endif; ?>
<?php /**PATH C:\Proyectos\proyecto para corregir }\proyecto actual\Proyecto-del-sena-\resources\views/usuario/delete.blade.php ENDPATH**/ ?>