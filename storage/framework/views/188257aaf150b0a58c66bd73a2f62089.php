...
<link rel="stylesheet" href="<?php echo e(asset('css/viewport-preview.css')); ?>">
...
unset($__errorArgs, $__bag); ?>" id="telefono" name="telefono" value="<?php echo e(old('telefono', $registro->telefono ?? '')); ?>" placeholder="Ejemplo: +591 70000000">
                        <input type="hidden" name="phone_code" id="phone_code" value="<?php echo e(old('phone_code', '')); ?>">
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    try {
        var vpScript = document.createElement('script'); vpScript.src = '<?php echo e(asset("js/viewport-preview.js")); ?>'; document.head.appendChild(vpScript);
            ...
            if (vpEl && vpEl.__vp && typeof vpEl.__vp.exportCropped === 'function') {
                try {
                    ...
                    try {
                        ...
                        try {
                            let overlay = vpEl.querySelector('.viewport-overlay-raster');
                            if (!overlay) {
                                overlay = document.createElement('img');
                                overlay.className = 'viewport-overlay-raster';
                                ...
                                vpEl.style.position = vpEl.style.position || 'relative';
                                vpEl.appendChild(overlay);
                            }
                            const displayRaster = await vpEl.__vp.exportCropped(displayPx);
                            if (displayRaster) {
                                overlay.src = displayRaster;
                                overlay.style.display = '';
                            }
                        } catch (e) {
                            console.warn('viewport overlay rasterization failed', e);
                        }
                    } catch (e) {
    ...
    if (editSave) {
        ...
        function getCroppedData() {
            ...
            const minCrop = parseInt('<?php echo e(config("avatar.crop_min", 100)); ?>', 10) || 100;
            const maxCrop = parseInt('<?php echo e(config("avatar.crop_max", 800)); ?>', 10) || 800;
